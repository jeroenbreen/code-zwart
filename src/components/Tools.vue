...
export default {
    ...
    setup() {
        ...
        const daysBeforeHospitalization = ref(store.state.daysBeforeHospitalization);
        const lengthOfStay = ref(store.state.lengthOfStay);
        const icPercentage = ref(store.state.icPercentage);

        watch(() => daysBeforeHospitalization.value, (value) => {
            store.commit("setProperty", { key: "daysBeforeHospitalization", value})
        });

        watch(() => lengthOfStay.value, (value) => {
            store.commit("setProperty", { key: "lengthOfStay", value})
        });

        watch(() => icPercentage.value, (value) => {
...
<template>
    <div class="Tools">
        ...
        <div class="Tools__set">
            <div class="Tools__label">
                Percentage besmettingen naar IC
            </div>
