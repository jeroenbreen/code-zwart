...
import { aggregate, stripTimeline, getRTimeline } from "@/utils/source";
...
export default {
    ...
    setup() {
        ...
        getJson("https://raw.githubusercontent.com/mzelst/covid-19/master/data-dashboards/cases_ggd_agegroups.csv").then((data) => {
            ...
            const today = aggregated[aggregated.length - 1].date;
            ...
            const days = 7 * weeks;
            const sourceTimeline = stripTimeline(aggregated, days);
            store.commit("setProperty", {key: "todayLabel", value: today});
